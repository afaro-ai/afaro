...
Where choosing the agent answer opens fields the manifest does not describe, do not carry `agent_value_literal` at all. Three manifests are in exactly that position, and the third shows what the real obstacle is. Its markup carries the boxes the agent answer reveals, hidden until that answer is picked: an agent first name, last name and email, all required. So the missing piece was never the capture. A manifest is a straight list of steps with no way to say that three of them run only when the profile carries that block, and filling the agent answer would leave three required boxes empty and the form would be refused. All three leave the field out and stop, and their notes say why. A capability that walks a run into an undescribed form is worse than the stop it replaces.
...
What a manifest needs to carry one: the decline or minimal-consent control in `selector`, with the visible label the dialog shows, and the dialog on a capture. Most of these open with the page, so `source_capture` shows them; one reached later in a flow goes in `additional_captures` like any other. A dialog nobody has captured gets no step. Two brokers stop a scan on their own defences today and neither carries one, because guessing at a control on a dialog nobody has seen is how a tool clicks Accept All on somebody's behalf. A third did until its opt-out turned out to run through an account rather than through the search.
